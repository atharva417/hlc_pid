...
class min_snap:
    ...
    def solve(self):
        ...
        self.J=((np.matmul(np.matmul(np.transpose(self.p_x),self.Q),self.p_x))) +((np.matmul(np.matmul(np.transpose(self.p_y),self.Q),self.p_y)))+((np.matmul(np.matmul(np.transpose(self.p_z),self.Q),self.p_z)))

        print(self.J,"cost")

    def grad_des(self,iter):
        self.t = tf.Variable(self.t)
        optimizer = tf.keras.optimizers.Adam(0.001)
        for i in range(iter):
            optimizer.minimize(cost, [self.t])
